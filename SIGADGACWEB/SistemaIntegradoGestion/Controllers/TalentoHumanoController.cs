...
namespace SistemaIntegradoGestion.Controllers
{
    public class TalentoHumanoController : Controller
    {
        ...
        public JsonResult ObtenerMaestroPersonal()
        {
            ...
            oMaestroPersonal = CD_TalentoHumano.Instancia.MaestroPersonalPorCedula(ousuario.CedulaIdentificacion);
            oListarMaestroPersonal.Add(oMaestroPersonal);
            if (oMaestroPersonal.PathFoto.Length > 0)
            {
                string urlDocumentos = Constantes.MaestroPersonalURL + ousuario.CedulaIdentificacion + @"\" + oMaestroPersonal.PathFoto;
        ...
        public ActionResult Editar_Maestro_Personal(tbMaestroPersonal modalPersonal, HttpPostedFileBase fileFoto)
        {
            ...
            try
            {
                ...
                if (ousuario.CedulaIdentificacion.Length > 0)
                {
                    if (fileFoto != null && fileFoto.ContentLength > 0)
                        modalPersonal.PathFoto = GuardaServidorArchivo(fileFoto, ousuario.CedulaIdentificacion);
                    ...
                    {
                        string urlDocumentos = Constantes.MaestroPersonalURL + ousuario.CedulaIdentificacion + @"\" + modalPersonal.PathFoto;
                        // Convert image to byte array
                        byte[] byteData = System.IO.File.ReadAllBytes(urlDocumentos);
                        //Convert byte arry to base64string
                        string imreBase64Data = Convert.ToBase64String(byteData);
